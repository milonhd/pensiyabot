...
async def cmd_start(message: types.Message):
    ...
    if message.from_user.id == ADMIN_ID:
        ...
        if expire_time and expire_time > time.time():
            await message.answer("👋 Добро пожаловать, {name}! У вас уже есть доступ.", reply_markup=materials_keyboard)
        else:
            welcome_text = (
                "👋 *Добро пожаловать, {name}, в бот «СВОЯ ПЕНСИЯ»* – твой персональный помощник на пути к достойной пенсии!\n"
