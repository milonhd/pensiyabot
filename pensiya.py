...
async def cmd_start(message: types.Message):
    ...
    first_name = user.first_name or "Пользователь"  # Используем имя или заглушку
    ...
    if message.from_user.id == ADMIN_ID:
        ...
        if expire_time and expire_time > time.time():
            await message.answer("👋 Добро пожаловать, {first_name}! У вас уже есть доступ.", reply_markup=materials_keyboard)
        else:
            welcome_text = (
                "👋 *Добро пожаловать, {first_name}, в бот «СВОЯ ПЕНСИЯ»* – твой персональный помощник на пути к достойной пенсии!\n"
